...
void ui_render(Renderer* renderer, enum nk_anti_aliasing aa)
{
    ...
    renderer_viewport(renderer, rect_min_dim(v2_zero(), v2((f32)state->width, (f32)state->height)));
    ...
    nk_foreach(cmd, &state->ctx)
    {
        ...
        switch (cmd->type)
        {
            ...
            case NK_COMMAND_SCISSOR:
            {
                ...
                f32 y = (f32)(state->height - s->h - s->y);
            ...
            case NK_COMMAND_POLYGON:
            {
                ...
                draw_polygon_outline(renderer, pts, (u32)p->point_count, color, (f32)p->line_thickness);
